...
class EncoderLayer(nn.Module):
    def __init__(self, input_channel, output_channel):
        ...
        self.encoding = nn.Sequential(
                nn.Conv2d(input_channel, output_channel, 5, stride=2, padding=2, bias=False),
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, kernel_size=5):
        ...
        self.decoding = nn.Sequential(
                nn.Conv2d(input_channel, output_channel, kernel_size, stride=1, padding=2, bias=False),
                nn.BatchNorm2d(output_channel),
                nn.ReLU()
                )

    def forward(self, x1, x2=None, concat=True):
        x1 = self.decoding(F.interpolate(x1, x2.shape[2:]))
